...
| "Skip the fingerprint, just hand-write the new artifact" | "OK, drafting." | "Hand-write skips N1+N2+N3 — N4 cosine will score against an unobserved baseline and `naga.fidelity.measured` will fire with `N=0`. Validator will reject the row. Proceed anyway?" |
| *(your own prior claim)* "no shape signal exists for this domain" | "Right, fall back to naming-only." | "I framed that too narrowly — N2 vocabulary TF-IDF works on prose targets even when N1 AST doesn't. Pivoting." |
...
| `inference-substrate.md` | Where recurring doubt-pass catches compound across plugins |
